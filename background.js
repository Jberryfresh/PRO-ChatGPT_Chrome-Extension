...
chrome.runtime.onInstalled.addListener(() => {
  chrome.contextMenus.create({ id: "ask-gpt", title: 'Ask PRO ChatGPT about: "%s"', contexts: ["selection"] });
});
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId !== "ask-gpt") return;
  try {
    const answer = await askOnce(info.selectionText || "Explain this page.");
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: (text) => {
        let box = document.getElementById("pro-gpt-box");
        if (!box) {
          ...
          const title = document.createElement("strong"); title.textContent = "PRO ChatGPT";
          ...
          box.appendChild(bar);
          const content = document.createElement("div"); content.id = "pro-gpt-content";
          box.appendChild(content);
          document.body.appendChild(box);
        }
        document.getElementById("pro-gpt-content").textContent = text;
      },
      args: [answer]
    });
...
chrome.runtime.onConnect.addListener((port) => {
  ...
  port.onMessage.addListener(async (msg) => {
    ...
    catch (e) { port.postMessage({ type: "error", message: e.message }); return; }
