...
document.getElementById("saveKey").addEventListener("click", async () => {
  await chrome.storage.sync.set({ OPENAI_API_KEY: keyEl.value.trim() });
  keyMsg.textContent = "Saved API key.";
});
(async function initTheme(){
  const { theme = "dark" } = await chrome.storage.sync.get("theme");
  themeEl.value = theme;
  themeEl.addEventListener("change", async () => {
    await chrome.storage.sync.set({ theme: themeEl.value });
  });
})();
...
async function loadPresets() {
  ...
  return (Array.isArray(presets) && presets.length) ? presets : DEFAULTS;
}
function renderPresets(list) {
  ...
  list.forEach((p, idx) => {
    ...
    del.addEventListener("click", async () => {
      list.splice(idx,1); await chrome.storage.sync.set({ presets: list }); renderPresets(list);
    });
    ...
    td1.appendChild(in1); td2.appendChild(in2); td3.appendChild(del);
...
document.getElementById("addPreset").addEventListener("click", async () => {
  ...
  list.push({ name, text });
  await chrome.storage.sync.set({ presets: list });
  pname.value = ""; ptext.value = "";
  renderPresets(list); presetMsg.textContent = "Preset added.";
});

document.getElementById("resetDefaults").addEventListener("click", async () => {
  await chrome.storage.sync.set({ presets: DEFAULTS });
  renderPresets([...DEFAULTS]);
  presetMsg.textContent = "Reset to defaults.";
});
